...
function calendar_info_next(req, res, next, iv){
  ...
  function goal_data (req, res, iv, userid, start_date, end_date, callback){
      ...
      ', goal.initValues as initValues FROM gym_member_goal as goal where'+
      ' ( (goal.startDate BETWEEN "'+start_date +'" and "'+end_date+'")'+
      ' Or ("'+start_date +'" BETWEEN goal.startDate and goal.endDate)'+
      ' Or ("'+end_date +'" BETWEEN goal.startDate and goal.endDate) )'+
      ' and goal.user="'+userid+'" and (goal.status="active"'+
        ...
        if (err){
          ...
          if(user_row.length<=0){
            ...
            for(let i=0;i<user_row.length;i++){
              ...
              user_row[i].goal_Date1 =  (user_row[i].goal_Date);
              user_row[i].goal_endDate1 =  (user_row[i].goal_endDate);
              try{
                if(goalDate!=undefined && goalDate!=null && goalDate!="" && goalDate.indexOf(' ')!=-1  &&
                  (goalendDate!=undefined && goalendDate!=null && goalendDate!="" && goalendDate.indexOf(' ')!=-1) ){
                     ...
                     send_goal_data.push({
                       ...
                       "newstartDate":user_row[i].goal_Date1,
                       "newendDate":user_row[i].goal_endDate1
                     });
                }
                      ...
                      if(nutrition_Promise[i].nutrition_Date == finalString[j].date){
                        if(flag){
                          tempNutrition.push({
                            ...
                            "workout_DateTime" : finalString[j].workout_DateTime,

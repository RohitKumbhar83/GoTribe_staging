    ...
    try{
      ...
      console.log('v   '+query_exec);
       ...
       if(err){
         ...
         let  schedule_list_query = 'SELECT gc.name as class_name, cs.schedule_type, cs.member_for, cs.assign_staff_mem,'+
                     ...
                     ' LEFT JOIN gym_member gm ON gm.id = cs.assign_staff_mem' +
        ...
        let addUserinQuery =  '';
         ...
         console.log(user_data.role_name +' role_name');
           ...
           condition = condition + ' AND csl.schedule_date >= "'+ start_date +'" AND csl.schedule_date <= "'+ end_date+  '"';
            condition = condition + ' ORDER BY csl.schedule_date ASC, csl.start_time, "%h:%i:%a"  DESC';
              ...
              if(err){
                ...
                if(sche_row.length<=0){
                  ...
                  try{
                    ...
                    response_data.output = send_view_data;
                    let enc   = encrypt_decrypt.encrypt(cryptkey, iv, JSON.stringify(response_data));
                    res.end(enc);
                  }
                }
                      ...
                      if(err){
                        ...
                        if(wait_data.length<=0){
                          ...
                          if( wait_details.length >= ( sche_details.total_capacity + sche_details.wait_list) ){
                            ...
                            connection_db.query(schedule_list_query,function(err,wait_data){
                              if(err){
                                // throw err;
                                let response_data = status_codes.db_error_0001;
                                console.log(response_data);
...
function setScheDates (subs_data_details){
  // check weekly limit
  // console.log(subs_data_details);
  try{
    let schedule_date = subs_data_details.schedule_date;
